...
const recommendation = document.getElementById('emotionRecommendation');
...
function updateRecommendation(emotion) {
  switch (emotion.toLowerCase()) {
    case 'senang':
      recommendation.textContent = 'Pertahankan mood positif! Dengarkan musik favoritmu atau berbagi kebahagiaan dengan orang lain.';
      moodIcon.className = 'fas fa-face-laugh-beam';
      break;
    case 'sedih':
      recommendation.textContent = 'Coba istirahat sejenak, dengarkan musik yang menenangkan, atau bicaralah dengan teman.';
      moodIcon.className = 'fas fa-face-sad-tear';
      break;
    case 'marah':
      recommendation.textContent = 'Tarik napas dalam-dalam, lakukan teknik relaksasi, atau jalan-jalan sebentar untuk meredakan emosi.';
      moodIcon.className = 'fas fa-face-angry';
      break;
    case 'netral':
      recommendation.textContent = 'Kamu dalam kondisi stabil. Gunakan waktu ini untuk fokus atau relaksasi ringan.';
      moodIcon.className = 'fas fa-meh';
      break;
    default:
      recommendation.textContent = 'Emosi belum terdeteksi. Pastikan kamera aktif.';
      moodIcon.className = 'fas fa-face-meh';
  }
}


// Contoh pemanggilan (gantilah dengan output dari face-api)
function simulateEmotionDetection() {
  const emotions = ['senang', 'sedih', 'marah', 'netral'];
  const detected = emotions[Math.floor(Math.random() * emotions.length)];
  emotionElement.textContent = detected.charAt(0).toUpperCase() + detected.slice(1);
  updateRecommendation(detected);
}

// Simulasi setiap 5 detik
